...
func SimpleSimulation() {
	...
	//tq.Push(newTran1)
	fmt.Println("A transaction for swap(X->Y) is randomly broadcast --------------")
	...
	fmt.Println("A transaction for swap(Y->X) is broadcast -----------------------")
	...
	//tq.Push(newTran3)
	//fmt.Printf("Now we have %d tran in the queue.\n", tq.Len())
	fmt.Println("A transaction for deposit is broadcast --------------------------")
	...
	fmt.Println("A transaction for withdraw is broadcast -------------------------")
	newTran4.Print() // Print
	//tq.Push(newTran4)
	//fmt.Printf("Now we have %d tran in the queue.\n", tq.Len())

	newTran4 = processTransaction(newTran4, &lp)
	fmt.Println("The 4th transaction for withdraw is processed -------------------")
	...
	liquidity.ComparePoolsPrint(initialLP, lp)
	fmt.Println("Test All Completed.")
}
...
func createRandomTrans(trade string, x int64, y int64, lt int64) liquidity.Transaction {
	...
	newCoinX = sdk.NewDecCoinFromDec("atom", newAmountX)
